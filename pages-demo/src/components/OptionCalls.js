...
function OptionCalls() {
    ...
    React.useEffect(() => {
        ...
        if (screnSize === 'desktop') {
            ...
            axios.get('http://127.0.0.1:8000/api/OptionCalls_data/Desktop')
                .then(function (response) {
                    // handle success
                    console.log('Desktop option call data - ', response);
        ...
        if (screnSize === 'tab') {
            ...
            // setChangeCall('Tab');
            axios.get('http://127.0.0.1:8000/api/OptionCalls_data/Tab')
                .then(function (response) {
                    // handle success
                    console.log('Tablet option call data - ', response);
                    setPageData(response.data)
                    setLoading(false)
                })
                .catch(function (error) {
        ...
        if (screnSize === 'mobile') {
            ...
            axios.get('http://127.0.0.1:8000/api/OptionCalls_data/Mobile')
                .then(function (response) {
                    // handle success
                    console.log('Mobile option call data - ', response);
                    setPageData(response.data)
                    setLoading(false)
                })
                .catch(function (error) {
                                ...
                                pageData.data.map((data, indexH) => (
                                    <tr key={indexH} className="hover:bg-gray-100 border-b border-gray-200 bg-white text-center py-4">                                        {
                                            data.map((item) => (
                                                ...
                                                // <td key={item} className="tbl-cell-pt max-w-[12px]">{parse(artytag)}</td>
